...
- The Android UI redesign is completed for the current MVP. It uses a dark sports analytics theme with near-black backgrounds, dark cards, and lime accents. Login/Register use floating in-app notifications and show/hide password controls. Match List opens on Upcoming, caches loaded tabs in `MatchListViewModel`, provides a manual `Обновить матчи` action, and visually separates actual upcoming matches from seeded demo matches. Match Details uses a compact tablet layout; Prediction Result and History use dark analytics cards; Profile uses a centered dashboard-style user card.
