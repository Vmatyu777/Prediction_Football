...
- Current endpoints: `GET /health`, `GET /db/health`, `GET /models`, `GET /matches`, `GET /matches/{match_id}`, `GET /matches/upcoming`, `GET /matches/recent`, `POST /predict`, `POST /predict/{match_id}`, `GET /predictions/{prediction_id}`.
...
- Match-based prediction uses SQLite runtime feature generation; do not retrain models or change final ML configurations from the API layer.
- SQLite-backed runtime feature preparation is implemented for match-based prediction.
...
- Runtime prediction features for match-based prediction are generated by `src/api/services/feature_service.py` from SQLite history, not from training feature CSV files.
...
- Android must not calculate ML features, access SQLite directly, run models locally, or implement reconciliation locally.
- Implemented Android screens: match list, match details, and prediction result.
