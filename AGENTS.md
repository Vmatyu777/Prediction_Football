...
- Production is served at `https://prediction-football.ru/` on TimeWeb Cloud through Nginx reverse proxy and Let's Encrypt certificates. DNS is managed through Cloudflare. Nginx proxies requests to the Dockerized backend on `http://127.0.0.1:8000`.
