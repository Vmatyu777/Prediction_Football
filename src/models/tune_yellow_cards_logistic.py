...
from src.features.feature_registry import YELLOW_CARDS_FEATURE_SETS  # noqa: E402
from src.models.train_yellow_cards import TARGET_COLUMN, build_time_split  # noqa: E402


FEATURE_DATA_PATH = PROJECT_ROOT / "data" / "interim" / "matches_features_v2.csv"
MODELS_DIR = PROJECT_ROOT / "models" / "yellow_cards"
TABLES_DIR = PROJECT_ROOT / "reports" / "tables" / "yellow_cards"
...
def build_logistic_model(feature_names: list[str], c_value: float, class_weight) -> Pipeline:
    return Pipeline(
        steps=[
...
def main() -> None:
    ...
    fitted_models: dict[str, tuple[str, list[str], Pipeline]] = {}
    tuning_rows = []

    for feature_set, feature_names in YELLOW_CARDS_FEATURE_SETS.items():
        ...
        for c_value in C_VALUES:
            for class_weight_name, class_weight in CLASS_WEIGHT_CONFIGS.items():
                ...
                model = build_logistic_model(feature_names, c_value, class_weight)
                model.fit(X_train, y_train)
                fitted_models[model_key] = (feature_set, feature_names, model)
    ...
    tuning_metrics.to_csv(TABLES_DIR / "yellow_cards_logistic_tuning_metrics.csv", index=False)
    ...
    eligible = validation_metrics[
        ...
        & (validation_metrics["yes_recall"] >= 0.35)
    ]
    ...
    selected_models = (
        ...
        .head(5)[["feature_set", "model"]]
        .to_dict("records")
    )

    threshold_rows = []
    for selected in selected_models:
        model_key = f"{selected['feature_set']}__{selected['model']}"
        feature_set, feature_names, model = fitted_models[model_key]
    ...
    threshold_metrics.to_csv(TABLES_DIR / "yellow_cards_logistic_threshold_validation.csv", index=False)

    threshold_eligible = threshold_metrics[
        (threshold_metrics["balanced_accuracy"] >= 0.52)
        & (threshold_metrics["no_recall"] >= 0.35)
        & (threshold_metrics["yes_recall"] >= 0.35)
    ]
    ...
    best_model_key = f"{best_threshold['feature_set']}__{best_threshold['model']}"
    best_feature_set, best_feature_names, best_model = fitted_models[best_model_key]
    best_threshold_value = float(best_threshold["threshold"])

    final_rows = evaluate_model_on_splits(
        feature_set=best_feature_set,
        model_name=f"{best_threshold['model']}__threshold_tuned",
        model=best_model,
        splits=splits,
    ...
    final_metrics.to_csv(TABLES_DIR / "yellow_cards_logistic_selected_threshold_metrics.csv", index=False)

    best_base = validation_metrics.sort_values(["balanced_accuracy", "f1"], ascending=False).iloc[0]
    best_base_key = f"{best_base['feature_set']}__{best_base['model']}"
    base_feature_set, base_feature_names, best_base_model = fitted_models[best_base_key]
    ...
    existing_metrics_path = TABLES_DIR / "yellow_cards_model_metrics.csv"
    ...
    comparison.to_csv(TABLES_DIR / "yellow_cards_final_controlled_comparison.csv", index=False)
    joblib.dump(best_model, MODELS_DIR / "logistic_regression_controlled_best.joblib")

    print("Yellow Cards LogisticRegression controlled tuning completed.")
