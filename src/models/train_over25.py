...
from src.features.feature_registry import OVER25_FEATURE_SETS  # noqa: E402


FEATURE_DATA_PATH = PROJECT_ROOT / "data" / "interim" / "matches_features_v2.csv"
MODELS_DIR = PROJECT_ROOT / "models" / "over25"
TABLES_DIR = PROJECT_ROOT / "reports" / "tables" / "over25"
FIGURES_DIR = PROJECT_ROOT / "reports" / "figures" / "over25"

TARGET_COLUMN = "Target_Over25"
...
def evaluate_binary_classifier(
    *,
    ...
    matrix = confusion_matrix(y_true, y_pred, labels=OVER25_LABELS)
        ...
        rows.append(
            {
                ...
                "over25_yes_rate": split_data[TARGET_COLUMN].mean(),
                "over25_no_rate": 1 - split_data[TARGET_COLUMN].mean(),
            }
        )
    pd.DataFrame(rows).to_csv(TABLES_DIR / "over25_time_split.csv", index=False)


def save_feature_lists() -> None:
    rows = []
    for feature_set, features in OVER25_FEATURE_SETS.items():
        rows.extend({"feature_set": feature_set, "feature": feature} for feature in features)
    pd.DataFrame(rows).to_csv(TABLES_DIR / "over25_feature_sets.csv", index=False)
...
def main() -> None:
    ...
    required_columns = sorted(
        ...
        | {feature for features in OVER25_FEATURE_SETS.values() for feature in features}
    )
    ...
    for feature_set, feature_names in OVER25_FEATURE_SETS.items():
    ...
    metrics.to_csv(TABLES_DIR / "over25_model_metrics.csv", index=False)
    pd.concat(report_frames, ignore_index=True).to_csv(
        TABLES_DIR / "over25_classification_reports.csv",
        index=False,
    )
    pd.concat(matrix_frames, ignore_index=True).to_csv(
        TABLES_DIR / "over25_confusion_matrices.csv",
        index=False,
    )
    ...
    print("Over2.5 training completed.")
    print("Time split:")
    print(pd.read_csv(TABLES_DIR / "over25_time_split.csv").to_string(index=False))
    print("Metrics:")
