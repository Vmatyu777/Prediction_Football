...
FIGURES_DIR = PROJECT_ROOT / "reports" / "figures" / "btts"
...
def confusion_matrix_frame(
    *,
    ...
    matrix = confusion_matrix(y_true, y_pred, labels=BTTS_LABELS)
    rows = []
        ...
        rows.append(
            {
                ...
                "btts_yes_rate": split_data[TARGET_COLUMN].mean(),
                "btts_no_rate": 1 - split_data[TARGET_COLUMN].mean(),
            }
        )
    pd.DataFrame(rows).to_csv(TABLES_DIR / "btts_time_split.csv", index=False)


def save_feature_lists() -> None:
    rows = []
    for feature_set, features in BTTS_FEATURE_SETS.items():
        rows.extend({"feature_set": feature_set, "feature": feature} for feature in features)
    pd.DataFrame(rows).to_csv(TABLES_DIR / "btts_feature_sets.csv", index=False)
...
def run_model(
    *,
    ...
    for split_name, split_data in splits.items():
        ...
        y_pred = pd.Series(raw_pred.reshape(-1) if hasattr(raw_pred, "reshape") else raw_pred, index=y_split.index).astype(int)
...
def main() -> None:
    ...
    required_columns = sorted(
        ...
        | {feature for features in BTTS_FEATURE_SETS.values() for feature in features}
    )
    ...
    for feature_set, feature_names in BTTS_FEATURE_SETS.items():
    ...
    metrics.to_csv(TABLES_DIR / "btts_model_metrics.csv", index=False)
    pd.concat(report_frames, ignore_index=True).to_csv(
        TABLES_DIR / "btts_classification_reports.csv",
        index=False,
    )
    pd.concat(matrix_frames, ignore_index=True).to_csv(
        TABLES_DIR / "btts_confusion_matrices.csv",
        index=False,
    )
    ...
    print("BTTS training completed.")
    print("Time split:")
    print(pd.read_csv(TABLES_DIR / "btts_time_split.csv").to_string(index=False))
    print("Metrics:")
    print(metrics.sort_values(["split", "f1"], ascending=[True, False]).to_string(index=False))
    print(
        f"Best validation model by F1: {best['feature_set']} / {best['model']} "
        f"({best['f1']:.4f})"
    )
