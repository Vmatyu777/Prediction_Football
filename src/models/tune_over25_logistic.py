...
from src.models.train_over25 import TARGET_COLUMN, build_time_split  # noqa: E402
...
def build_logistic_model(c_value: float, class_weight) -> Pipeline:
    return Pipeline(
        steps=[
            ("preprocessor", build_preprocessor(OVER25_FEATURE_SETS["v1_only"])),
            (
                "model",
                LogisticRegression(
                    C=c_value,
...
def main() -> None:
    ...
    feature_names = OVER25_FEATURE_SETS["v1_only"]
    ...
    tuning_metrics.to_csv(TABLES_DIR / "over25_logistic_tuning_metrics.csv", index=False)

    validation_metrics = tuning_metrics[tuning_metrics["split"] == "validation"].copy()
    eligible = validation_metrics[
        (validation_metrics["balanced_accuracy"] >= 0.54)
        & (validation_metrics["no_recall"] >= 0.40)
        & (validation_metrics["yes_recall"] >= 0.40)
    ]
    ...
    threshold_metrics.to_csv(TABLES_DIR / "over25_logistic_threshold_validation.csv", index=False)

    threshold_eligible = threshold_metrics[
        (threshold_metrics["balanced_accuracy"] >= 0.54)
        & (threshold_metrics["no_recall"] >= 0.40)
        & (threshold_metrics["yes_recall"] >= 0.40)
    ]
    ...
    final_metrics.to_csv(TABLES_DIR / "over25_logistic_selected_threshold_metrics.csv", index=False)
    ...
    existing_metrics_path = TABLES_DIR / "over25_model_metrics.csv"
    ...
    comparison.to_csv(TABLES_DIR / "over25_final_controlled_comparison.csv", index=False)
    joblib.dump(best_model, MODELS_DIR / "logistic_regression_controlled_best.joblib")

    print("Over2.5 LogisticRegression controlled tuning completed.")
