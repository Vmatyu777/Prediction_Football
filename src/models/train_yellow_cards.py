...
FIGURES_DIR = PROJECT_ROOT / "reports" / "figures" / "yellow_cards"

TARGET_COLUMN = "Target_YellowCards_Over35"
CATEGORICAL_FEATURES = ["Division", "HomeTeam", "AwayTeam"]
RANDOM_STATE = 42
YELLOW_CARDS_LABELS = [0, 1]
...
def evaluate_binary_classifier(
    *,
    ...
    matrix = confusion_matrix(y_true, y_pred, labels=YELLOW_CARDS_LABELS)
        ...
        rows.append(
            {
                ...
                "yellow_cards_over35_yes_rate": split_data[TARGET_COLUMN].mean(),
                "yellow_cards_over35_no_rate": 1 - split_data[TARGET_COLUMN].mean(),
            }
        )
    pd.DataFrame(rows).to_csv(TABLES_DIR / "yellow_cards_time_split.csv", index=False)


def save_feature_lists() -> None:
    rows = []
    for feature_set, features in YELLOW_CARDS_FEATURE_SETS.items():
        rows.extend({"feature_set": feature_set, "feature": feature} for feature in features)
    pd.DataFrame(rows).to_csv(TABLES_DIR / "yellow_cards_feature_sets.csv", index=False)
...
def main() -> None:
    ...
    required_columns = sorted(
        ...
        | {feature for features in YELLOW_CARDS_FEATURE_SETS.values() for feature in features}
    )
    ...
    metrics.to_csv(TABLES_DIR / "yellow_cards_model_metrics.csv", index=False)
    pd.concat(report_frames, ignore_index=True).to_csv(
        TABLES_DIR / "yellow_cards_classification_reports.csv",
        index=False,
    )
    pd.concat(matrix_frames, ignore_index=True).to_csv(
        TABLES_DIR / "yellow_cards_confusion_matrices.csv",
        index=False,
    )
    ...
    print("Yellow Cards Over3.5 training completed.")
    print("Time split:")
    print(pd.read_csv(TABLES_DIR / "yellow_cards_time_split.csv").to_string(index=False))
    print("Metrics:")
